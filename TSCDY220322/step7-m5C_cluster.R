...
data=read.table('./03.survial/m5Cmerge.txt',header=T,sep="\t",row.names=1)
data=as.matrix(data)
if(T){
  #聚类
  maxK=9
  ...
  write.table(clusterOut, file="./04.cluster/Cluster.txt", sep="\t", quote=F, col.names=F)
  
  cluster=read.table("./04.cluster/Cluster.txt", header=T, sep="\t", check.names=F, row.names=1)
  row.names(cluster) <- gsub('\\.', '-',row.names(cluster))
  cli=read.table("./03.survial/LIHC_m5C.txt", header=T, sep="\t", check.names=F, row.names=1)
  cli <- cli[,c(1:2)]
  ...
  pdf(file="./04.cluster/survival.pdf",onefile = FALSE,width=6,height=5)
  print(surPlot)
  dev.off()
}

exp=read.table('./03.survial/m5Cmerge.txt', header=T, sep="\t", check.names=F, row.names=1)
exp=t(exp)
cluster=read.table("./04.cluster/Cluster.txt", header=T, sep="\t", check.names=F, row.names=1)
row.names(cluster) <- gsub('\\.', '-',row.names(cluster))
...
expCluster <- read.csv(file = '123.csv',header = T,row.names = 1)
...
plot1=pheatmap(data,
         ...
         fontsize_row=6,
         fontsize_col=6)
ggsave("./04.cluster/heatmap.jpg",plot =plot1, height=7, width=9,dpi = 1000)
ggsave("./04.cluster/heatmap.pdf",plot =plot1, height=7, width=9,dpi = 1000)
