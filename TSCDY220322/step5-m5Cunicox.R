...
#对基因进行循环，找出预后相关的基因
rt <- read.table('./03.survial/LIHC_m5C.txt', header=T, sep="\t", check.names=F,row.names = 1)
picDir <- './03.survial/'
outTab=data.frame()
km=c()
...
write.table(outTab,file="./03.survial/uniCox.txt",sep="\t",row.names=F,quote=F)

if(T){
  load('./03.survial/LIHC.RData')
  outTab=rbind(geneNames=colnames(geneExp), geneExp)
  write.table(outTab, file="./03.survial/m5Cmerge.txt", sep="\t", quote=F, col.names=F)
  gene.exp  <-  read.table("./03.survial/m5Cmerge.txt",header=T,sep="\t",check.names = F,row.names = 1)
  gene.group <- read.table("./00.data/m5c_gene.txt",header=T,sep="\t")
  gene.cox <- read.table("./03.survial/uniCox.txt",header=T,sep="\t")
  ...
  hua <- "./03.survial/network.pdf"
  nodefile <- "./03.survial/network.node.txt"
  edgefile <- "./03.survial/network.edge.txt"
