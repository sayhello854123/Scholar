...
m6A1 <- read.table('./00.data/PAAD/04.meger/m6A_merge.txt',header = T,row.names = 1,sep = '\t',check.names = F)
m6A <- t(m6A1)
dir.create('./add/PAAD')
dir <- './add/PAAD/'
...
write.table(outTab,file="./add/PAAD_cor.txt",sep="\t",row.names=F,quote=F)
