...
if(T){
  #读取输入文件,并对输入文件进行整理
  rt=read.table('00.data/STAD/04.meger/m6A_merge.txt', header=T, sep="\t", check.names=F)
  rt=as.matrix(rt)
  rownames(rt)=rt[,1]
  exp=rt[,2:ncol(rt)]
  ...
  write.table(pcaPredict, file="./00.data/STAD/08.PCA/newTab.xls", quote=F, sep="\t")
  ...
  pdf(file="./06.step6/06.PCA/PCA.pdf", height=5, width=6.5)
  ...
  ggsave(filename = "./06.step6/06.PCA/PCA.jpg",plot =p1,
