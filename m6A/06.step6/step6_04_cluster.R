...
data=read.table("./00.data/STAD/04.meger/m6A_merge.txt",header=T,sep="\t",check.names = F,row.names = 1)
data=as.matrix(data)
...
results=ConsensusClusterPlus(data,
                             ...
                             pItem=0.8,
                             pFeature=1,
                             title='./06.step6/03.cluster_heatmap/',
                             clusterAlg="pam",
                             distance="pearson",
...
if(T){
  ...
  write.table(clusterOut, file="./00.data/STAD/06.cluster/m6aCluster.txt", sep="\t", quote=F, col.names=F)
  
  
  cluster=read.table("./00.data/STAD/06.cluster/m6aCluster.txt", header=T, sep="\t", check.names=F, row.names=1)
  ...
  surPlot
  pdf(file="./06.step6/03.cluster_heatmap/survival.pdf",onefile = FALSE,width=7,height=6.5)
  print(surPlot)
  dev.off()
}  

if(T){
  exp=read.table("./00.data/STAD/04.meger/m6A_merge.txt", header=T, sep="\t", check.names=F, row.names=1)
  exp=t(exp)
  cluster=read.table('./00.data/STAD/06.cluster/m6aCluster.txt', header=T, sep="\t", check.names=F, row.names=1)
  row.names(cluster) <- gsub('\\.', '-',row.names(cluster))
  ...
  rownames(expCluster)=gsub("(.*?)\\_(.*?)", "\\2", rownames(expCluster))
  cli=read.csv("./00.data/STAD/04.meger/ALL_CLI.csv", row.names = 1, header = T)
  ...
  ggsave("./06.step6/03.cluster_heatmap/heatmap.jpg",plot =plot1, height=7, width=9,dpi = 1000)
  ggsave("./06.step6/03.cluster_heatmap/heatmap.pdf",plot =plot1, height=7, width=9,dpi = 1000)
}
