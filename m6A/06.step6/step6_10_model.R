...
#读取表达文件，并对输入文件整理
rt=read.table('./00.data/STAD/09.VENN/interGeneExp.txt', header=T, sep="\t", check.names=F)
rt=as.matrix(rt)
rownames(rt)=rt[,1]
exp=rt[,2:ncol(rt)]
...
cli=read.table("./00.data/STAD/04.meger/STAD_m6A.txt", header=T, sep="\t", check.names=F, row.names=1)
cli <- cli[,c(1:2)]
...
write.table(outTab,file="./00.data/STAD/11.Model/uniCox.xls",sep="\t",row.names=F,quote=F)
uniSigExp=rt[,sigGenes]
uniSigExp=cbind(id=row.names(uniSigExp),uniSigExp)
write.table(uniSigExp,file="./00.data/STAD/11.Model/uniSigExp.txt",sep="\t",row.names=F,quote=F)

##lasso##
rt=read.table("./00.data/STAD/11.Model/uniSigExp.txt",header=T,sep="\t",row.names=1,check.names=F)     #读取文件
...
pdf("./06.step6/09.model/lambda.pdf")
plot(fit, xvar = "lambda", label = TRUE)
dev.off()

cvfit <- cv.glmnet(x, y, family="cox", maxit = 1000)
pdf("./06.step6/09.model/cvfit.pdf")
plot(cvfit)
...
train=rt[,lassoGene]
group2 <-grep('GSE34942.*',row.names(rt1))
GSE34942 <- rt1[group2,]
group3 <-grep('GSE15459.*',row.names(rt1))
GSE15459 <- rt1[group3,]
TEST <- rbind(GSE34942,GSE15459)
test <- TEST[,lassoGene]
lassoSigExp=train
...
write.table(outTab,file="./00.data/STAD/11.Model/multiCox.xls",sep="\t",row.names=F,quote=F)
...
write.table(cbind(id=rownames(cbind(train[,outCol],riskScore,risk)),cbind(train[,outCol],riskScore,risk)),
            file="./00.data/STAD/11.Model/m6Ascore_train.txt",
            sep="\t",
            quote=F,
            row.names=F)
#输出test组风险文件
...
write.table(cbind(id=rownames(cbind(test[,outCol],riskScoreTest,riskTest)),cbind(test[,outCol],riskScore=riskScoreTest,risk=riskTest)),
            file="./00.data/STAD/11.Model/m6Ascore_test.txt",
            sep="\t",
            quote=F,
            row.names=F)

  
  sur_train=read.table("./00.data/STAD/11.Model/m6Ascore_train.txt", header=T, sep="\t", check.names=F, row.names=1)
  ...
  sur_test=read.table("./00.data/STAD/11.Model/m6Ascore_test.txt", header=T, sep="\t", check.names=F, row.names=1)
  ...
  write.table(outTab, file="./00.data/STAD/11.Model/m6Ascore_train_group1.txt", sep="\t", quote=F, col.names=F)
  outTab1=rbind(id=colnames(sur_test), sur_test)
  write.table(outTab1, file="./00.data/STAD/11.Model/m6Ascore_test_group1.txt", sep="\t", quote=F, col.names=F)
  ...
  data <- "./00.data/STAD/11.Model/m6Ascore_train_group1.txt"
  plot <- './06.step6/09.model/survival_train1.pdf'
  surplot(data,plot)
  
  data <- "./00.data/STAD/11.Model/m6Ascore_test_group1.txt"
  plot <- './06.step6/09.model/survival_test1.pdf'
  surplot(data,plot)
  ...
  inputFile <- "./00.data/STAD/11.Model/m6Ascore_train_group1.txt"
  outPdf <- './06.step6/09.model/rocTrain1.pdf'
  rocplot(inputFile,outPdf)
  
  inputFile <- "./00.data/STAD/11.Model/m6Ascore_test_group.txt"
  outPdf <- './06.step6/09.model/rocTest1.pdf'
  rocplot(inputFile,outPdf)
