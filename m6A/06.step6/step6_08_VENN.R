...
if(T){
  ...
  #读取输入文件，并对输入文件整理
  rt=read.table('./00.data/STAD/04.meger/merge.txt', header=T, sep="\t", check.names=F)
  rt=as.matrix(rt)
  rownames(rt)=rt[,1]
  exp=rt[,2:ncol(rt)]
  ...
  allDiffGenes=c()
  dir.create('./00.data/STAD/09.VENN/')
  dir <- './00.data/STAD/09.VENN/'
  ...
  pdf(file="./06.step6/07.venn/venn.pdf", width=5, height=5)
  grid.draw(venn.plot)
  dev.off()
  ...
  write.table(file="./00.data/STAD/09.VENN/interGene.txt",interGenes,sep="\t",quote=F,col.names=F,row.names=F)
  
  #保存交集基因的表达量
  interGeneExp=data[interGenes,]
  interGeneExp=rbind(id=colnames(interGeneExp), interGeneExp)
  write.table(interGeneExp, file="./00.data/STAD/09.VENN/interGeneExp.txt", sep="\t", quote=F, col.names=F)
}