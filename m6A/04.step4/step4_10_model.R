...
write.table(outTab,file="./00.data/COAD/11.Model/uniCox.xls",sep="\t",row.names=F,quote=F)
uniSigExp=rt[,sigGenes]
uniSigExp=cbind(id=row.names(uniSigExp),uniSigExp)
write.table(uniSigExp,file="./00.data/COAD/11.Model/uniSigExp.txt",sep="\t",row.names=F,quote=F)

##lasso##
rt=read.table("./00.data/COAD/11.Model/uniSigExp.txt",header=T,sep="\t",row.names=1,check.names=F)     #读取文件
...
pdf("./04.step4/09.model/lambda.pdf")
plot(fit, xvar = "lambda", label = TRUE)
dev.off()

cvfit <- cv.glmnet(x, y, family="cox", maxit = 1000)
pdf("./04.step4/09.model/cvfit.pdf")
plot(cvfit)
...
write.table(outTab,file="./00.data/COAD/11.Model/multiCox.xls",sep="\t",row.names=F,quote=F)
...
write.table(cbind(id=rownames(cbind(train[,outCol],riskScore,risk)),cbind(train[,outCol],riskScore,risk)),
            file="./00.data/COAD/11.Model/m6Ascore_train.txt",
            sep="\t",
            quote=F,
            row.names=F)
#输出test组风险文件
...
write.table(cbind(id=rownames(cbind(test[,outCol],riskScoreTest,riskTest)),cbind(test[,outCol],riskScore=riskScoreTest,risk=riskTest)),
            file="./00.data/COAD/11.Model/m6Ascore_test.txt",
            sep="\t",
            quote=F,
            row.names=F)

  
  sur_train=read.table("./00.data/COAD/11.Model/m6Ascore_train.txt", header=T, sep="\t", check.names=F, row.names=1)
  ...
  sur_test=read.table("./00.data/COAD/11.Model/m6Ascore_test.txt", header=T, sep="\t", check.names=F, row.names=1)
  ...
  write.table(outTab, file="./00.data/COAD/11.Model/m6Ascore_train_group.txt", sep="\t", quote=F, col.names=F)
  outTab1=rbind(id=colnames(sur_test), sur_test)
  write.table(outTab1, file="./00.data/COAD/11.Model/m6Ascore_test_group.txt", sep="\t", quote=F, col.names=F)
  ...
  data <- "./00.data/COAD/11.Model/m6Ascore_train_group.txt"
  plot <- './04.step4/09.model/survival_train.pdf'
  surplot(data,plot)
  
  data <- "./00.data/COAD/11.Model/m6Ascore_test_group.txt"
  plot <- './04.step4/09.model/survival_test.pdf'
  surplot(data,plot)
  ...
  inputFile <- "./00.data/COAD/11.Model/m6Ascore_train_group.txt"
  outPdf <- './04.step4/09.model/rocTrain.pdf'
  rocplot(inputFile,outPdf)
  
  inputFile <- "./00.data/COAD/11.Model/m6Ascore_test_group.txt"
  outPdf <- './04.step4/09.model/rocTest.pdf'
  rocplot(inputFile,outPdf)
}
