...
if(T){
  ...
  gene.exp  <-  read.table("./00.data/COAD/04.meger/m6A_merge.txt",header=T,sep="\t",check.names = F,row.names = 1)
  gene.group <- read.table("./00.data/m6A_gene.txt",header=T,sep="\t")
  gene.cox <- read.table("./00.data/COAD/05.survival/uniCox.txt",header=T,sep="\t")
  ...
  hua <- "./04.step4/02.network/network.pdf"
  nodefile <- "./00.data/COAD/05.survival/network.node.txt"
  edgefile <- "./00.data/COAD/05.survival/network.edge.txt"
