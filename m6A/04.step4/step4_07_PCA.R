...
if(T){
  #读取输入文件,并对输入文件进行整理
  rt=read.table('00.data/COAD/04.meger/m6A_merge.txt', header=T, sep="\t", check.names=F)
  rt=as.matrix(rt)
  rownames(rt)=rt[,1]
  exp=rt[,2:ncol(rt)]
  ...
  write.table(pcaPredict, file="./00.data/COAD/08.PCA/newTab.xls", quote=F, sep="\t")
  ...
  pdf(file="./04.step4/06.PCA/PCA.pdf", height=5, width=6.5)
  ...
  p1 <- ggplot(data = PCA, aes(PC1, PC2)) + geom_point(aes(color = m6Acluster)) +
    ...
    annotate("text",x=PCA.mean$PC1, y=PCA.mean$PC2, label=PCA.mean$m6Acluster, cex=7)+
    theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())
  ggsave(filename = "./04.step4/06.PCA/PCA.jpg",plot =p1,
