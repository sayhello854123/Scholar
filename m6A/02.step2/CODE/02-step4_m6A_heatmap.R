...
if(T){
  ...
  write.table(clusterOut, file="./00.data/ESCA/06.cluster/m6aCluster.txt", sep="\t", quote=F, col.names=F)
...
if(T){
  exp=read.table("./00.data/PAAD/04.meger/m6A_merge.txt", header=T, sep="\t", check.names=F, row.names=1)
  exp=t(exp)
  cluster=read.table('./00.data/PAAD/06.cluster/m6aCluster.txt', header=T, sep="\t", check.names=F, row.names=1)
  row.names(cluster) <- gsub('\\.', '-',row.names(cluster))
  ...
  cli=read.csv("./00.data/PAAD/04.meger/all_clinical.csv", row.names = 1, header = T)
  ...
  ggsave("./02.step2/03.PAAD_cluster/heatmap.jpg",plot =plot1, height=7, width=9,dpi = 1000)
  ggsave("./02.step2/03.PAAD_cluster/heatmap.pdf",plot =plot1, height=7, width=9)
}
