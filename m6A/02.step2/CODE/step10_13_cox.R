train=read.table("./00.data/PAAD/11.Model/m6Ascore_train.txt", header=T, sep="\t", check.names=F, row.names=1)
train <- train[,c(1,2,14)]
colnames(train) = c("futime", "fustat","m6A")
test=read.table("./00.data/PAAD/11.Model/m6Ascore_test.txt", header=T, sep="\t", check.names=F, row.names=1)
test <- test[,c(1,2,14)]
colnames(test) = c("futime", "fustat","m6A")
score <- rbind(train,test)
metabolize=read.table('./00.data/PAAD/07.GSVA/ssGSEA.result.txt', header=T, sep="\t", check.names=F, row.names=1)
...
write.table(sigGeneExp, file="./02.step2/10.cor_PAAD/PAAD_cox.txt", sep="\t", quote=F, col.names=F)

rt=read.table('./02.step2/10.cor_PAAD/PAAD_cox.txt',header=T,sep="\t",row.names=1,check.names=F)
gene=rownames(rt)
hr=sprintf("%.3f",rt$"HR")
...
pdf(file='./02.step2/10.cor_PAAD/PAAD_cox.pdf', width = 8.5, height =5.5)
n=nrow(rt)
nRow=n+1
ylim=c(1,nRow)
