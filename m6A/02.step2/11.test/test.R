...
for(i in 1:n){
  #############对数据进行分组#############
  inTrain<-createDataPartition(y=rt[,3], p=176/293, list=F)
  train<-rt[inTrain,]
  test<-rt[-inTrain,]
  ...
  if((pValue<0.05) & (roc$AUC[1]>0.7)&(roc$AUC[2]>0.67)&(roc$AUC[2]>0.65) & 
     (pValueTest<0.05) & (rocTest$AUC[1]>0.7)&(rocTest$AUC[2]>0.65)&(rocTest$AUC[3]>0.65)){
    #输出分组结果
    write.table(trainOut,file="./02.step2/11.test/data.train.txt",sep="\t",quote=F,row.names=F)
    write.table(testOut,file="./02.step2/11.test/data.test.txt",sep="\t",quote=F,row.names=F)
    #输出单因素结果
    write.table(outUniTab,file="./02.step2/11.test/uni.trainCox.txt",sep="\t",row.names=F,quote=F)
    write.table(uniSigExpOut,file="./02.step2/11.test/uni.SigExp.txt",sep="\t",row.names=F,quote=F)
    
    #lasso结果
    write.table(lassoSigExpOut,file="./02.step2/11.test/lasso.SigExp.txt",sep="\t",row.names=F,quote=F)
    pdf("./02.step2/11.test/lasso.lambda.pdf")
    plot(fit, xvar = "lambda", label = TRUE)
    dev.off()
    pdf("./02.step2/11.test/lasso.cvfit.pdf")
    plot(cvfit)
    abline(v=log(c(cvfit$lambda.min,cvfit$lambda.1se)), lty="dashed")
    dev.off()
    #lasso结果
    write.table(m6Ascore_trainOut,file="./02.step2/11.test/train.txt",sep="\t",quote=F,row.names=F)
    write.table(m6Ascore_testOut,file="./02.step2/11.test/test.txt",sep="\t",quote=F,row.names=F)
    break
  }
}
