library(GEOquery)
library(limma)
library(tidyverse)
library(sva)
library(AnnoProbe)
library(tidyverse)
##GEO
if(T){
  Sys.setenv("VROOM_CONNECTION_SIZE" = 524288 * 2)#GSE102238
  gset <- getGEO('GSE62452', destdir=".",
                 AnnotGPL = F,     ## 注释文件
                 getGPL = F) 
  a=gset[[1]]
  dat1=exprs(a)
  dim(dat1)
  ids <- AnnoProbe::idmap('GPL6244') 
  colnames(ids)=c('probe_id','symbol')  
  ids=ids[ids$symbol != '',]
  ids=ids[ids$probe_id %in%  rownames(dat1),]
  dat1=dat1[ids$probe_id,] 
  ids$median=apply(dat1,1,median) 
  ids=ids[order(ids$symbol,ids$median,decreasing = T),]
  ids=ids[!duplicated(ids$symbol),]
  dat1=dat1[ids$probe_id,]
  rownames(dat1)=ids$symbol
  rt1 <- dat1
  metadata=pData(a)
  clinical = data.frame(gsm=metadata[,2],
                        #id = metadata[,1],
                        tissue=trimws(sapply(as.character(metadata$characteristics_ch1),function(x) strsplit(x,":")[[1]][2])),
                        fustat=trimws(sapply(as.character(metadata$characteristics_ch1.4),function(x) strsplit(x,":")[[1]][2])),
                        futime=trimws(sapply(as.character(metadata$characteristics_ch1.3),function(x) strsplit(x,":")[[1]][2])),
                        grade = trimws(sapply(as.character(metadata$characteristics_ch1.1),function(x) strsplit(x,":")[[1]][2])),
                        Stage = trimws(sapply(as.character(metadata$characteristics_ch1.2),function(x) strsplit(x,":")[[1]][2]))
                        
  )
  cli <- clinical[clinical$tissue=="Pancreatic tumor", ]
  rownames(cli) <- cli[,1]
  #clinical <- cli
  gene=read.table('./00.data/m6A_gene.txt', header=T, sep="\t", check.names=F)
  sameGene=intersect(as.vector(gene[,1]), rownames(rt1))
  clinical <-read.table('./00.data/PAAD/02.GSE62452/clinical.txt', header=T, sep="\t", check.names=F,row.names = 1)
  same <- intersect(row.names(clinical),colnames(rt1))
  rt1 <- rt1[,same]
  outTab=rbind(geneNames=colnames(rt1), rt1)
  write.table(outTab, file="./00.data/PAAD/02.GSE62452/GSE62452.txt", sep="\t", quote=F, col.names=F)
  rownames(clinical) <- clinical[,1]
  outTab1=rbind(id_name=colnames(clinical), clinical)
  write.table(outTab1, file="./00.data/PAAD/02.GSE62452/clinical.txt", sep="\t", quote=F, col.names=F)
}