...
library(timeROC)
#读取表达文件，并对输入文件整理
rt=read.table('./01.step1/11.test/data.train.txt', header=T, sep="\t", check.names=F,row.names = 1)
rt1 <- read.table('./01.step1/11.test/data.test.txt', header=T, sep="\t", check.names=F,row.names = 1)
#对基因进行循环，找出预后相关的基因
pFilter=0.05
outTab=data.frame()
...
write.table(outTab,file="./01.step1/11.test/uniCox.xls",sep="\t",row.names=F,quote=F)
uniSigExp=rt[,sigGenes]
uniSigExp=cbind(id=row.names(uniSigExp),uniSigExp)
write.table(uniSigExp,file="./01.step1/11.test/uniSigExp.txt",sep="\t",row.names=F,quote=F)

##lasso##
rt=read.table("./01.step1/11.test/uniSigExp.txt",header=T,sep="\t",row.names=1,check.names=F)     #读取文件
...
pdf("./01.step1/11.test/lambda.pdf")
plot(fit, xvar = "lambda", label = TRUE)
dev.off()

cvfit <- cv.glmnet(x, y, family="cox", maxit = 1000)
pdf("./01.step1/11.test/cvfit.pdf")
plot(cvfit)
...
write.table(lassoSigExp1,file="./01.step1/11.test/TCGA_lassoSigExp.txt",sep="\t",row.names=F,quote=F)
...
write.table(m6Ascore_trainOut,file="./01.step1/11.test/train.txt",sep="\t",row.names=F,quote=F)
...
write.table(m6Ascore_testOut,file="./01.step1/11.test/test.txt",sep="\t",row.names=F,quote=F)
...
data <- "./01.step1/11.test/train.txt"
plot <- './01.step1/11.test/survival_train.pdf'
surplot(data,plot)

data <- "./01.step1/11.test/test.txt"
plot <- './01.step1/11.test/survival_test.pdf'
surplot(data,plot)
...
inputFile <- "./01.step1/11.test/train.txt"
outPdf <- './01.step1/11.test/rocTrain.pdf'
rocPlot(inputFile,outPdf)

inputFile <- "./01.step1/11.test/test.txt"
outPdf <- './01.step1/11.test/rocTest.pdf'
rocPlot(inputFile,outPdf)
