...
data=read.table('./00.data/LIHC/megerData/m6Amerge.txt',header=T,sep="\t",row.names=1)
data=as.matrix(data)
if(T){
#聚类
maxK=9
results=ConsensusClusterPlus(data,
                             ...
                             pItem=0.8,
                             pFeature=1,
                             title='./01.step1/06.m6ALIHC_cluster/',
                             clusterAlg="pam",
                             distance="pearson",
...
write.table(clusterOut, file="./00.data/LIHC/cluster/m6aCluster.txt", sep="\t", quote=F, col.names=F)
...
pdf(file="./01.step1/06.m6ALIHC_cluster/survival.pdf",onefile = FALSE,width=7,height=6.5)
print(surPlot)
dev.off()
}

if(T){
exp=read.table('./00.data/LIHC/megerData/m6Amerge.txt', header=T, sep="\t", check.names=F, row.names=1)
exp=t(exp)
cluster=read.table('./00.data/LIHC/cluster/m6aCluster.txt', header=T, sep="\t", check.names=F, row.names=1)
row.names(cluster) <- gsub('\\.', '-',row.names(cluster))
...
plot1=pheatmap(data,
         ...
         fontsize_row=6,
         fontsize_col=6)
pdf("./01.step1/06.m6ALIHC_cluster/heatmap.pdf", height=5, width=8)
ggsave("./01.step1/06.m6ALIHC_cluster/heatmap.jpg",plot =plot1, height=7, width=9,dpi = 1000)
}
