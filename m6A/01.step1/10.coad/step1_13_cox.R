  ...
  write.table(sigGeneExp, file="./01.step1/09.m6aLIHC_COR/LICH_cox.txt", sep="\t", quote=F, col.names=F)
  
  rt=read.table('./01.step1/09.m6aLIHC_COR/LICH_cox.txt',header=T,sep="\t",row.names=1,check.names=F)
  gene=rownames(rt)
  hr=sprintf("%.3f",rt$"HR")
  ...
  pdf(file='./01.step1/09.m6aLIHC_COR/LIHC_cox.pdf', width = 8.5, height =5.5)
  n=nrow(rt)
  nRow=n+1
  ylim=c(1,nRow)
