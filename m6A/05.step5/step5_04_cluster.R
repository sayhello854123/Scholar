...
data=read.table("./00.data/READ/04.meger/m6A_merge.txt",header=T,sep="\t",check.names = F,row.names = 1)
data=as.matrix(data)
...
results=ConsensusClusterPlus(data,
                             ...
                             pItem=0.8,
                             pFeature=1,
                             title='./05.step5/03.cluster_heatmap/',
                             clusterAlg="pam",
                             distance="euclidean",
                             seed=123456,
                             plot="png")
#save(results,file ='./00.data/READ/06.cluster/cluster.RData')
...
if(T){
  ...
  write.table(clusterOut, file="./00.data/READ/06.cluster/m6aCluster1.txt", sep="\t", quote=F, col.names=F)
  ...
  #write.table(clusterOut1, file="./00.data/READ/06.cluster/ALL.txt", sep="\t", quote=F, col.names=F)
  
  
  
  
  rt <- read.table('./00.data/READ/06.cluster/ALL1.txt',sep = '\t',header = T,check.names=F, row.names=1)
  #生存差异统计
  ...
  pdf(file="./05.step5/03.cluster_heatmap/survival.pdf",onefile = FALSE,width=7,height=6.5)
  print(surPlot)
  dev.off()
}

if(T){
  exp=read.table("./00.data/READ/04.meger/m6A_merge.txt", header=T, sep="\t", check.names=F, row.names=1)
  exp=t(exp)
  cluster=read.table('./00.data/READ/06.cluster/m6aCluster.txt', header=T, sep="\t", check.names=F, row.names=1)
  row.names(cluster) <- gsub('\\.', '-',row.names(cluster))
  ...
  cli=read.table("./00.data/READ/04.meger/ALL_cli.txt", sep="\t",row.names = 1, header = T,check.names=F)
  ...
  ggsave("./05.step5/03.cluster_heatmap/heatmap.jpg",plot =plot1, height=7, width=9,dpi = 1000)
  ggsave("./05.step5/03.cluster_heatmap/heatmap.pdf",plot =plot1, height=7, width=9,dpi = 1000)
}
